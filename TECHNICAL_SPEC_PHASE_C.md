...
1. **非复合年判据 —— ✅ 已解决（2026-09-23，读 S1 后）**：
   ...
   **执行口径**：
   ...
   ⚠️ **第三轮补注（与论文正文的一处冲突）**：论文正文 `paper_text.txt:415` 举例时写
   "**the non-compound year 2000** … return periods of about 60 years"，即论文把 **2000 年**
   当作非复合年；而本项目 S1 判读把 2000 判为"有（波罗的海 + 地中海 1 点）"、未纳入任何一档。
   ⇒ 主口径之外**应把 2000 单列为敏感性 C**（论文点名年份，仅用于核对图6c 的 60 年锚点），
   并在图注中说明该年份归属与论文示例的关系。

   三种（连 2000 共四种）取法都要跑并报告；预期结论稳健（图6a 是"2023 ≈ 40 天 vs 非复合年 < 5 天"）。
...
> ✅ **两项待定规则已于 2026-09-23 落实（第三轮更正——原"待定"清单过期）**：
> 1. **非复合年的可操作判据 —— 已解决**：读论文 Supplementary Fig. S1 逐格判读得到
>    "地中海零复合活动" 8 年（1984/1985/1986/1988/1989/1991/1993/1998，主口径），
>    另设 10 年（+1992/1996）与 11 年（+1987）两档敏感性；论文的 10/11 无法唯一恢复。
>    **见本节步骤 9 的完整口径表**与 `results/方法与证据.md` §3。
> 2. **100 km 海岸缓冲 —— 已实现**：`python/coastal_buffer.py` +
>    `config.COASTAL_BUFFER_KM = 100.0` / `COASTAL_BUFFER_REGION`，
>    实测 1952/4492 陆地格点落入缓冲区（最大距海 99.7 km）。**见本节步骤 8**。
>
> ⬜ **仍然悬空的一项**：图6 分析域的**区域口径**（默认沿用 fig1j 框 lat 30–47 / lon 5–42
> 含黑海；论文 Fig.6 字面只写 "Mediterranean coast"）——开工前需拍板，见步骤 8 的备选口径表。
